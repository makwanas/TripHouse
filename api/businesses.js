...
const {deleteBusinessById} = require("../models/business");
const {replaceBusinessById} = require("../models/business");
 ...
 */
router.put('/:id', requireAuthentication, requireBusinessOwnerId, verifyTokenWithEmail, async (req, res, next) => {
    if (validateAgainstSchema(req.body, BusinessSchema)) {
        try {
 ...
 */
router.delete('/:id', requireAuthentication, requireBusinessOwnerId, verifyTokenWithEmail, async (req, res, next) => {
    try {
        const deleteSuccessful = await deleteBusinessById(req.params.id)
        if (deleteSuccessful) {
            res.status(204).end()
        } else {
            next()
